...
function doSolveTsume(sfen, timeLimit) {
  isThinking = true; tsumeMode = true;
  sendEngine('usinewgame');
  sendEngine(`position sfen ${sfen}`);
  sendEngine(`go mate ${timeLimit || 'infinite'}`);
  emitToClient('engine:status', { status: 'thinking', message: '詰将棋解析中…' });
}
...
function startEngine() {
  ...
  rl.on('line', (line) => {
    ...
    if (line.startsWith('info') && tsumeMode && line.includes('score mate')) {
      const m = line.match(/score mate (\d+)/);
      if (m) emitToClient('tsume:progress', { mateIn: parseInt(m[1]) });
      return;
    }
    ...
    if (line.startsWith('checkmate ')) {
      isThinking = false; tsumeMode = false;
      const rest = line.slice(10).trim();
      if (rest === 'nomate' || rest === 'timeout') {
        emitToClient('tsume:failed', {});
      } else {
        const moves = rest.split(' ').filter(Boolean);
        emitToClient('tsume:solution', { moves });
      }
      emitToClient('engine:status', { status: 'ready', message: '詰将棋解析完了' });
      if (pendingTsume)          { const t = pendingTsume;          pendingTsume          = null; doSolveTsume(t.sfen, t.timeLimit); }
...
function handleClientEvent(event, data) {
  switch (event) {
    ...
    case 'solve_tsume': {
      const { sfen, timeLimit } = data || {};
      if (!sfen || !engineReady || autoState !== 'idle') break;
      pendingAnalysis = null; pendingOptions = null;
      if (isThinking) { pendingTsume = { sfen, timeLimit }; sendEngine('stop'); } else doSolveTsume(sfen, timeLimit);
      break;
    }
...
async function main() {
  ...
  sigSocket.on('connect', () => {
    ...
    dataChannel = null;
    // フロントエンドの接続を待たずにエンジンを起動しておく（スタンバイ状態の準備）
    // 既に起動済みの場合は startEngine() 内でスキップされる
    startEngine();
  });
